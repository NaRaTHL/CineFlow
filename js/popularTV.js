document.addEventListener('DOMContentLoaded', () => {
  ...
  function toggleShowInList(key, show) {
    const list = getStorageList(key);
    const index = list.findIndex(item => item.id === show.id);
    if (index === -1) list.push(show);
  ...
  function renderShows(shows) {
    ...
    shows.forEach(show => {
      ...
      const inWatchlist = isInList('watchlist', show.id);
      const inWatched = isInList('watched', show.id);

      const html = `
        <div class="tv-item w-48 flex-shrink-0 group cursor-pointer relative transition-all duration-300 ease-out opacity-0 translate-y-5">
          <div class="overflow-hidden rounded-lg shadow-lg relative">
            <a href="${basePath}page/detail.html?id=${show.id}&type=tv">
            ...
            <div class="absolute top-2 left-2 flex items-start justify-between w-[calc(100%-1rem)]">
              ...
              <div class="flex flex-col gap-1 items-end">
                <button class="watchlist-btn p-1.5 rounded-md ${inWatchlist ? 'bg-red-600' : 'bg-gray-800 hover:bg-red-600'}" data-id="${show.id}" title="Add to Watchlist">
                  <svg xmlns="http://www.w3.org/2000/svg" fill="currentColor" class="w-4 h-4" viewBox="0 0 24 24">
                    <path d="M5 3a2 2 0 0 0-2 2v16l9-4 9 4V5a2 2 0 0 0-2-2H5z"/>
                  </svg>
                </button>

                <button class="watched-btn p-1.5 rounded-md ${inWatched ? 'bg-green-600' : 'bg-gray-800 hover:bg-green-600'}" data-id="${show.id}" title="${inWatched ? 'Viewed ✅' : 'Mark as Watched 👁️'}">
                  ${inWatched 
      ...
      tvGrid.insertAdjacentHTML('beforeend', html);
    });

    // Fade-in animation
    setTimeout(() => {
      tvGrid.querySelectorAll('.tv-item').forEach(el => {
        el.classList.remove('opacity-0', 'translate-y-5');
      });
    }, 100);

    // Watchlist button
    tvGrid.querySelectorAll('.watchlist-btn').forEach(btn => {
      btn.addEventListener('click', e => {
        e.stopPropagation();
        const id = parseInt(btn.dataset.id);
        const show = allShows.find(m => m.id === id);
        toggleShowInList('watchlist', show);
    ...
    // Watched button
    tvGrid.querySelectorAll('.watched-btn').forEach(btn => {
      btn.addEventListener('click', e => {
        e.stopPropagation();
        const id = parseInt(btn.dataset.id);
        const show = allShows.find(m => m.id === id);
        toggleShowInList('watched', show);
  ...
  async function loadShows() {
    ...
    const newShows = await fetchTVShows(currentPage);
    allShows = allShows.concat(newShows);
    renderShows(newShows);
