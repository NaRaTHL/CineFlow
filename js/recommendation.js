document.addEventListener("DOMContentLoaded", () => {
  ...
  function isInList(key, id) {
    return getStorageList(key).some((item) => item.id === id);
  }
  ...
  function renderMovies(movies) {
    if (!movies.length) {
      grid.innerHTML = `<p class="text-gray-400 text-center col-span-full">No results found.</p>`;
      return;
    }
        ...
        return `
          <div class="movie-item w-48 mx-auto group cursor-pointer relative">
            <div class="overflow-hidden rounded-xl shadow-lg relative bg-gray-900">
              <a href="${basePath}page/detail.html?id=${movie.id}&type=${
          movie.title ? "movie" : "tv"
              ...
              <div class="absolute top-2 left-2">
                <svg class="w-10 h-10" viewBox="0 0 40 40">
                  ...
                  <text x="50%" y="50%" text-anchor="middle" dy="0.35em" fill="white" font-size="12">${rating.toFixed(
                    0
                  )}%</text>
                </svg>
              </div>
              ...
              <div class="absolute top-2 right-2 flex flex-col gap-1">
                <button class="watchlist-btn p-1.5 rounded-md ${
                  inWatchlist ? "bg-red-600" : "bg-gray-800 hover:bg-red-600"
                }" data-id="${movie.id}" title="Watchlist">
                  <svg xmlns="http://www.w3.org/2000/svg" fill="currentColor" class="w-4 h-4" viewBox="0 0 24 24">
                    <path d="M5 3a2 2 0 0 0-2 2v16l9-4 9 4V5a2 2 0 0 0-2-2H5z"/>
                  </svg>
                </button>

                <button class="watched-btn p-1.5 rounded-md ${
                  inWatched ? "bg-green-600" : "bg-gray-800 hover:bg-green-600"
                }" data-id="${movie.id}" title="${
          inWatched ? "Viewed ✅" : "Mark as Watched 👁️"
        }">
                  ${
                    inWatched
  ...
  // --- Fetch Recommendations from Seeds ---
  async function fetchRecommendedFromSeed(type, topMovies) {
    const recommended = [];
    for (const movie of topMovies.slice(0, 5)) {
      try {
        const res = await fetch(
          `${BASE_URL}/${type}/${movie.id}/recommendations?api_key=${API_KEY}&language=en-US&page=1`
        );
        const data = await res.json();
        if (data.results) recommended.push(...data.results);
      } catch (e) {
        console.warn("Failed to fetch recommendations for", movie.title);
      }
    }
    return recommended;
  }
  ...
  loadMoreBtn.addEventListener("click", () => {
    ...
    loadMoreBtn.style.display = currentPage < totalPages ? "inline-block" : "none";
    localStorage.setItem("cine_recommendation_page", currentPage);
    window.scrollTo({ top: resultsSection.offsetTop + resultsSection.clientHeight - 200, behavior: "smooth" });
  });
